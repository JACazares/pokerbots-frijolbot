...
class RoundState(
    ...
    def get_bounty_hits(self):
        ...
        """
        cards0 = self.hands[0] + (
            [] if self.street == 0 else self.deck.peek(self.street)
        )
        cards1 = self.hands[1] + (
            [] if self.street == 0 else self.deck.peek(self.street)
        )
        ...
        return (
            self.bounties[0] in [cardNames[card.rank] for card in cards0],
    ...
    def proceed(self, action):
        ...
        if isinstance(action, CheckAction):
            ...
            ) or self.button > 1:  # both players acted
    ...
    def run(self):
        ...
        for round_num in range(1, NUM_ROUNDS + 1):
            ...
            if round_num % ROUNDS_PER_BOUNTY == 1:
                ...
                bounties = [
                    cardNames[random.randint(0, 12)],
                    cardNames[random.randint(0, 12)],
                ]
                self.log.append(
                    f"Bounties reset to {bounties[0]} for player {players[0].name} and {bounties[1]} for player {players[1].name}"
                )
            self.run_round(players, bounties)
            self.log.append(
                "Winning counts at the end of the round: " + STATUS(players)
            )
